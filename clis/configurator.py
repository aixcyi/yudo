...
def configurate_frpc(pattern: str, filename: str, list_all: bool, delete_it: bool):
    ...
    if list_all:
        ...
        for name, info in find_frp_configs(path, 'frpc').items():
            if name == '':
                command = cmd(configurate, configurate_frpc)
            else:
                command = cmd(configurate, configurate_frpc, '-c', name)
        ...
        try:
            cfp = find_frp_config(path, 'frpc', filename)
