...
@cli.option('-o', '--offset', 'offsets', metavar='LEFT[~RIGHT][,BASE]', type=RawRange(timedelta | date), multiple=True,
...
@cli.option('-r', '--regex', help='过滤不能完全匹配正则表达式的(格式化后的)日期。')
@cli.option('-m', '--millisecond', 'is_ms_base', help='以毫秒为单位（默认是秒）进行穷举。')
@cli.option('-F', '--force', is_flag=True, help='不提示数量，直接穷举输出所有时间。')
def gendt(
        fmt: str,
    ...
    moments = filter(lambda d: re.fullmatch(pattern, d), moments) if pattern else moments
